...
import { Activity, ArrowRight, ArrowUpRight, Bell, Box, CheckCheck, ChevronDown, ChevronLeft, ChevronRight, CircleDollarSign, CircleHelp, Clock3, CreditCard, EllipsisVertical, FileDown, Flag, Headset, Home, Info, LayoutDashboard, MapPin, Menu, MessageSquareMore, Package, Pencil, Route, Search, Send, Settings, ShieldCheck, Timer, Trash2, Truck, Users, Waypoints, X, type LucideIcon } from "lucide-react";
import { money, STATUS_LABELS, tripRoute, type DashboardSnapshot, type Shipment, type Person, type Trip, type Dispute, type Offer, type SupportUserDetails, type SupportActivityEntry, type AgentScoreboardEntry } from "@passenger/core";
  ...
  return <div className={`app-shell app-shell-${view}`}>
    ...
    <div className={`main-shell main-shell-${view}`}>
      ...
      <main id="main-content" className={`main-content main-content-${view}`}>
        ...
        {view === "offers" && <Panel title="Chat" count={offersShown.length}><div className="notice">Internal messaging tools and delivery conversation oversight will appear here. This screen is still being aligned to the Figma chat design.</div><div className="member-list">{offersShown.map(o => <OfferRow key={o.id} offer={o} snapshot={snapshot} now={now} onSelect={setSelection} />)}</div>{!offersShown.length && <Empty title="No chats yet" description="Chat and conversation moderation records will appear here." />}</Panel>}
        {(view === "verifications" || view === "compliance") && (selection?.type === "user"
          ? <FigmaUserDetailPage user={people.find(person => person.id === selection.id)} shipments={shipments} walletTransactions={snapshot.walletTransactions ?? []} reviews={snapshot.reviews ?? []} onAction={submit} onUpdateTier={onUpdateTier} viewerIsCompliance={snapshot.viewer?.role === "compliance"} onBack={() => setSelection(null)} />
    ...
    {selection && selection.type !== "user" && <Drawer selection={selection} snapshot={snapshot} onClose={() => setSelection(null)} onSelect={setSelection} onAction={submit} renderOperations={renderOperations} />}
  </div>;
}
...
function Drawer({ selection, snapshot, onClose, onSelect, onAction, renderOperations }: { selection: Selection; snapshot: DashboardSnapshot; onClose: () => void; onSelect: (s: Selection) => void; onAction: ActionHandler; renderOperations?: Props["renderOperations"] }) {
  const ref = useRef<HTMLDialogElement>(null);
  useEffect(() => { const previousFocus = document.activeElement as HTMLElement; const dialog = ref.current; dialog?.showModal(); const overflow = document.body.style.overflow; document.body.style.overflow = "hidden"; return () => { dialog?.close(); document.body.style.overflow = overflow; previousFocus?.focus(); }; }, []);
...
function ParcelDetail({ shipment: s, snapshot, onSelect, onAction }: { shipment: Shipment; snapshot: DashboardSnapshot; onSelect: (s: Selection) => void; onAction: ActionHandler }) {
  ...
  return <><div className="parcel-detail-title"><span className="parcel-icon"><Box size={25} /></span><div><h3>{s.reference}</h3><span>Created {fullDate(s.createdAt)}</span></div><Status status={s.status} /></div><div className="detail-route"><div><MapPin size={18} /><span><small>PICKUP</small><strong>{s.origin}</strong></span></div><span className="route-connector" /><div><MapPin size={18} /><span><small>DESTINATION</small><strong>{s.destination}</strong></span></div></div><h3 className="detail-heading">Declared contents</h3><p className="description-box">{s.description}</p><dl className="detail-grid"><Detail label="Category" value={s.category} /><Detail label="Weight" value={`${s.weightKg} kg`} /><Detail label="Declared value" value={money(s.valueNaira)} /><Detail label="Gross fee" value={money(s.feeNaira)} /><Detail label="Payment status" value={humanize(s.paymentStatus)} /><Detail label="Safety consent" value={s.safetyConsent ? "Confirmed" : "Not recorded — review required"} /><Detail label="Ready for pickup" value={fullDate(s.readyAt)} /><Detail label="Delivery deadline" value={fullDate(s.deliveryDeadline)} /><Detail label="Pickup meeting point (private)" value={s.pickupInstructions} /><Detail label="Drop-off meeting point (private)" value={s.dropoffInstructions} /><Detail label="Receiver (private)" value={s.receiverName} /><Detail label="Receiver phone (private)" value={s.receiverPhone} /></dl>{s.quote && <dl className="detail-grid"><Detail label={`Platform fee (${s.quote.platformFeePercent}%)`} value={money(s.quote.platformFeeKobo / 100)} /><Detail label="Traveller net" value={money(s.quote.travellerNetKobo / 100)} /></dl>}
    ...
    {s.reviewNote && <><h3 className="detail-heading">Latest review explanation</h3><p className="description-box">{s.reviewNote}</p></>}{s.status === "rejected" && <div className="notice">Changes requested. The sender can edit evidence or details and resubmit; no cancellation has been invented. Review controls return after resubmission.</div>}{s.status === "pending_review" && <ReviewForm type="review" id={s.id} onAction={onAction} />}
